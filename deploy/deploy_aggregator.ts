...
export default async function (hre: HardhatRuntimeEnvironment) {
  console.log(`Running deploy script for the Aggregator contract`);
  ...
  const artifact = await deployer.loadArtifact(
    "contracts/core/Aggregator.sol:Aggregator"
  );
  ...
  const verificationId = await hre.run("verify:verify", {
    ...
    contract: "contracts/core/Aggregator.sol:Aggregator",
