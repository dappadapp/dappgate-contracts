...
export default async function (hre: HardhatRuntimeEnvironment) {
    console.log(`Running deploy script for the DappadTokenClaim contract`);

    const wallet = new Wallet(process.env.PRIVATE_KEY!);

    const deployer = new Deployer(hre, wallet);
    const artifact = await deployer.loadArtifact("DappadTokenClaim");
    ...
    */
    ...
    const verificationId = await hre.run("verify:verify", {
        ...
        contract: "contracts/DappadTokenClaim.sol:DappadTokenClaim",
    ...
    const filePath = path.join(dirPath, 'tokenclaim.json');
