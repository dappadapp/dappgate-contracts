...
contract DappGate is Ownable, ERC721, NonblockingLzApp {
    uint256 public counter;
    uint public cost = 1.09 ether;
    ...
    constructor(
        address _endpoint
    ) ERC721('DappGate by DappLabs', 'DLGATE') NonblockingLzApp(_endpoint) {
    ...
    function mint() external payable {
        ...
        require(success);
        _mint(msg.sender,uint(keccak256(abi.encodePacked(counter, block.difficulty, block.timestamp))) % 10000000);
        unchecked {
            ++counter;
        }
    }
