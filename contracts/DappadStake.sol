 ...
 */
    ...
    )
    {
        if (hasStaked[user]) {
            return (
                ...
                deposits[user].stakeIndexes[deposits[user].stakeIndexes.length - 1],
    ...
    function _stake(address from, uint256 amount) private returns (bool) {
        if (!hasStaked[from]) {
            hasStaked[from] = true;
            uint64[] memory stakeIndexes = new uint64[](1);
            stakeIndexes[0] = index;
            ...
            deposits[from].depositAmount = newAmount;
            deposits[from].depositTime = block.timestamp;
            deposits[from].endTime = block.timestamp.add(lockDuration);
            deposits[from].stakeIndexes.push(index);
            deposits[from].rewards = rewards;
        }
    ...
    returns (uint256) {
        ...
        (uint256 amount, uint256 depositTime, uint64 userIndex) = (
            ...
            deposits[from].stakeIndexes[deposits[from].stakeIndexes.length - 1]
        );
        uint256 time;
        uint256 interest;
        ...
        return amount.sub(deposits[from].depositAmount);
    }
    ...
    function _calculateRewardsFromLastIndex(address from, uint64 lastIndex) private view returns (uint256) {
        if (lastIndex == index) {
            return 0;
        }
        uint256 rewards = 0;
        uint256 depositTime = deposits[from].depositTime;
        for (uint64 i = lastIndex + 1; i < index; i++) {
            uint256 interest = _calculateInterestFromIndex(from, i);
            rewards = rewards.add(interest);
            depositTime = rates[i].time;
        }
        return rewards;
    }

    function _calculateInterestFromIndex(address from, uint64 i) private view returns (uint256) {
            uint256 depositTime = deposits[from].depositTime;
            uint256 _lockduration = deposits[from].endTime.sub(depositTime);
            uint256 time = rates[i].time.sub(depositTime);
            uint256 interest = deposits[from].depositAmount.mul(rates[i - 1].interestRate).mul(time).div(
                _lockduration.mul(10000)
            );
            return interest;
    }
